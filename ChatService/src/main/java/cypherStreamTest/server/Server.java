package cypherStreamTest.server;
...
public class Server {
    ...
    public void initCipher(int mode, Key key, String algorithm) throws InvalidKeyException, NoSuchPaddingException, NoSuchAlgorithmException {
    ...
    private byte[] decryptMessage(byte[] messageEncrypted, Key key) {
        try{
            initCipher(Cipher.DECRYPT_MODE, key, "RSA");
    ...
    private byte[] encryptMessage(byte[] messageToEncrypt, Key key) {
        try{
            initCipher(Cipher.ENCRYPT_MODE, key, "RSA");
    ...
    private boolean sendMessage(byte[] message, Socket socket, Key key) {
        try {
            ...
            byte [] encodedKey = (byte [])objectInputStream.readObject();
            SecretKey sessionKey = new SecretKeySpec(encodedKey, 0, encodedKey.length, "AES");
            System.out.println(sessionKey);
            objectInputStream.close();

            //Now with the session key let's send a message
            /*
            connectServer();
            initCipher(Cipher.ENCRYPT_MODE, sessionKey, "AES");
            CipherOutputStream cipherOutputStream = new CipherOutputStream(socket.getOutputStream(), cipher);
            ObjectOutputStream objectOutputStream = new ObjectOutputStream(cipherOutputStream);
            objectOutputStream.writeObject("The session key is finally working. Hurray!");
